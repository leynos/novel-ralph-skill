...
`main`. The task is test-and-docs only: it generalises the existing
...
a `chapter-99` artefact the coherent baseline never materialises, both carry a
...
Four sites independently serialise a `Reconciliation` into the same base dict —
...
segregation guard), but the *serialisation* of a `Reconciliation` into the base
...
- **Proposed fix:** Give `Reconciliation` a single payload projection beside its
  ...
  vocabulary split is preserved); only the `Reconciliation`-to-dict serialisation
  is centralised, so `check` and `reconcile` cannot drift on payload shape. This
