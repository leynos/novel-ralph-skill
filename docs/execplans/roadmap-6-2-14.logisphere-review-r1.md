...
- ROLLBACK classification: `reconcile.py::_classify_pending_turn`
  ...
  outside `_RECOMPUTABLE_BASENAMES` (`{state.toml, log.md}`). `done.flag`
  qualifies. Verified.
...
- Bijection (`_check_manifest_disk_bijection`, lines 124-159) keys on
  `chapter-NN/` directory presence via `_on_disk_chapter_numbers`; a `.tmp`
  file inside an existing chapter dir adds no on-disk chapter number. Verified.
...
- Producer: `document.py::pending_turn(path, *, operation, paths)` accepts a
  free-form `operation` and `paths`; the never-landed `done.flag` `Examples`
  row of `torn_turn_rollback.feature` already drives `operation="mark-done"`,
...
- "No external-library research load-bearing" section: the runner path is fully
  ...
  plan's memory-free framing is correct — there is nothing to firecrawl-cite
  because nothing external is on the path. Verified.
...
- The devguide misattribution the plan targets in Work item 4 is real: the
  partial-landed bullet (developers-guide.md ~lines 1026-1029) labels the
  family "(task 6.2.13)", but roadmap 6.2.12 is the partial-landed `draft.md`
  cell and 6.2.13 is the never-landed `done.flag` cell. The plan's correction
  (6.2.12 for `draft.md`, 6.2.14 for `done.flag`) is accurate.
...
- A1 (Doggylump). Work item 3 is conditionally a no-op ("folds into Work item 2
  ...
  audit trail is unambiguous about why there may be three commits or four.
...
- A4 (Pandalump). The `_RESIDUE_BODY` should be a non-empty, distinctive marker
  so the byte-for-byte preservation assertion is meaningful (an empty body
  would still pass but prove less). The plan says "a short residue marker
  string" — acceptable; just ensure it is non-empty.
